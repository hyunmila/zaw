...
f = open("lab2/office/temporalROI.txt", "r")
line = f.readline()
...
I_prev_read = cv2.imread("lab2/office/input/in%06d.jpg"%roi_start)
I_prev_g = cv2.cvtColor(I_prev_read, cv2.COLOR_BGR2GRAY)
I_prev = I_prev_g

for i in range(roi_start, roi_end, step):
    I = cv2.imread("lab2/office/input/in%06d.jpg"%i)
    # cv2.imshow("I", I)
    ...
    (T, thresh) = cv2.threshold(I_diff, 9, 255, cv2.THRESH_BINARY)
    ...
    G = cv2.imread("lab2/office/input/in%06d.jpg"%i)
...
# Precission:  0.9853068280034573 Recall:  0.06051919095397356 F1:  0.1140342102630789